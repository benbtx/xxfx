...
function handle(char, transferCase) {
    ...
    for (let p in chinesePointCode) {
        if (chinesePointCode[p].indexOf(charPoint) > -1) {
            ...
            // handle transferCase
            // 处理 transferCase
            if (transferCase && transferCase === "toUpperCase") {
                char = char.toUpperCase();
            } else if (transferCase && transferCase === "upperfirst") {
...
export function py(value, transferCase, breakStr = "_") {
    /**
     * @param value 需要转换的字符
     * @param transferCase 转换后的样式，默认全小写，toUpperCase-> 全大写 upperfirst->首字母大写
     * @param breakStr 每个字拼音间的分割符号
     */
    ...
    if (length) {
        for (let i = 0; i < length; i++) {
            let char = chars[i];
            chars[i] = handle(char, transferCase);
        }
        value = chars.join(breakStr);
    }
    return value;
}
